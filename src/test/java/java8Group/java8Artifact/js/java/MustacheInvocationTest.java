...
public class MustacheInvocationTest {
	 ...
	 */
	@Test
	public final void testMain() throws FileNotFoundException, ScriptException,
		...
		String contactJson = "{" + "\"contact\": {" + "\"name\": \""
				+ contact.get("name") + "\", \"emails\": [" + "\""
				+ contact.get("email1") + "\", \"" + contact.get("email2")
				+ "\"" + "]}}";
		...
		System.out.println("\n Evaluating template " + template
				...
				+ "\n with result as \n" + result);
		Assert.assertTrue("Contains name",
				result.contains(contact.get("name")));
		Assert.assertTrue("Contains email1",
				result.contains(contact.get("email1")));
		Assert.assertTrue("Contains email2",
				result.contains(contact.get("email2")));
	}
